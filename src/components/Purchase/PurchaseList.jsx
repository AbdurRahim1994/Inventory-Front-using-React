...
const PurchaseList = () => {
    ...
    const TotalPurchase = useSelector((state) => state?.purchase?.TotalPurchase)

    const OnPageChangeClick = async (event) => {
        await PurchaseListRequest(event.selected + 1, perPage, search)
    }

    const OnPerPageChange = async (event) => {
        setPerPage(event.target.value);
        await PurchaseListRequest(1, event.target.value, search)
    }
    ...
    const OnSearchClick = async () => {
        await PurchaseListRequest(1, perPage, search)
    }
                        ...
                        <div className='card-body'>
                            <div className='row'>
                                <div className='col-lg-4'>
                                    <h4 className='font-weight-bolder'>Purchase List</h4>
                                </div>
                                            ...
                                            <tbody>
                                                {
                                                    PurchaseList.map((item, i) =>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                {
                                                                    item?.Supplier.map((sup, i) =>
                                                                        <p className='text-start text-xs'>{sup?.name}</p>
                                                                    )
                                                                }
                                                            </td>
                                ...
                                <div className='col-lg-12'>
                                    <nav aria-label="Page navigation example">
                                        <ReactPaginate
                                            ...
                                            pageCount={TotalPurchase / perPage}
