...
import { ReturnReportByDateRequest } from '../../apiRequests/ReportAPIRequest';
...
const ReturnReport = () => {
    ...
    const OnCreateReport = async () => {
        ...
        if (IsEmpty(fromDate)) {
            ...
            const res = await ReturnReportByDateRequest(fromDate, toDate)
        }
    }

    const ReturnByDateList = useSelector((state) => state?.report?.ReturnByDateList)

    const OnExport = async (exportType) => {
        const data = ReturnByDateList[0]['Rows']
        const reportData = []
        data.map((item) => {
            const listItem = {
        ...
        exportFromJSON({ data: reportData, fileName: "Return Report", exportType })
    }
    ...
    return (
        <div className='container-fluid'>
            <div className='row'>
                <div className='col-lg-12'>
                    <div className='card'>
                        <div className='card-body'>
                            <h4 className='font-weight-bolder'>Return Report</h4>
                ...
                {
                    ReturnByDateList.length > 0 ?
                        <div className='col-lg-12 mt-3'>
                            <div className='card'>
                                <div className='card-body'>
                                    {/* <h4>Total : <CurrencyFormat value={ReturnByDateList[0]['TotalAmount'][0]['TotalAmount']} thousandSeparator={true} prefix='$' displayType='text'></CurrencyFormat></h4> */}
                                    <h4>Total : {ReturnByDateList[0]['TotalAmount'][0]['TotalAmount']}</h4>
                                    <div className='row'>
